...
function container()
{
	...
	function changeCode(newCode,newDesc)
	{
		...
		// //Take User input and use to update garmet code and layer name.
		 function updateCode(newCode,newDesc)
		 {
		 	var localValid = true;
	...
	function batchPrompt()
	{
		...
		log.bp("Running script:::Change_Code.jsx version " + version)
		...
		var w = new Window("dialog","Batch All Open Documents?")
			...
			var ngcGroup = w.add("group");
				...
				var usrInput = ngcGroup.add("edittext", undefined, "eg. FD_217Y");
					usrInput.characters = 20;
			...
			var btnGroup = w.add("group");
				var oneBut = btnGroup.add("button", undefined, "Just this document.");
					oneBut.onClick = function()
					{
						...
						justOne(usrInput.text,descInput.text);
						w.close();
					}
				var allBut = btnGroup.add("button", undefined, "All open documents.");
					allBut.onClick = function()
					{
						...
						allDocs(usrInput.text,descInput.text);
						w.close();
					}
		...
		function allDocs(newCode,newDesc)
		{
			log.l("Batching all open documents.");
			log.l("Open documents are:");
			for(var x in app.documents.length)
			{
				log.l(app.documents[x]);
			}
			while(app.documents.length>0)
			{

				changeCode(newCode,newDesc);
				app.activeDocument.close(SaveOptions.SAVECHANGES);
			}
		}


		function justOne(newCode,newDesc)
		{
